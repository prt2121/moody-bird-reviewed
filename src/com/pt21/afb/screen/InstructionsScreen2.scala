 ...
 */
class InstructionsScreen2(game: AngryFlappyBird, val gesture: Option[GestureHandler]) extends Screen {
  ...
  override def show(): Unit = {
    ...
    val footer = new Label("<Tab to finish>", AssetLoader.smallSkin)
    val table = new Table
    table.setFillParent(true)
    table.setSkin(AssetLoader.skin)
    stage.addActor(table)
    table.add("Tap to Poop").expandX
    table.row
    table.add(footer).spaceTop(30)
    gesture.map({ controller =>
      controller.onTap(() => {
        //Note: can't new GameScreen() here! because we'll lose the reference to the screen.
